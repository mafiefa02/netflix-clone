...
{
    /* TODO:
    1. Add functionality to the search icon.
    2. Add functionality to the notification icon.
    3. Add hover effects to the profile icon.
    */
}
...
function Navbar() {
    ...
    return (
        <nav className={showNavBackground ? `fixed w-full top-0 bg-[#141414] transition-all duration-700 z-[99]` : `fixed w-full top-0 transition-all duration-700 delay-100 z-[99]`}>
            <div className="flex justify-between items-center py-[18px] px-16">
                ...
                <div className="flex-1 justify-start px-10 items-center">
                    {navbarButtons.map((items) => (
                        <a key={items.key} href={items.href} className="text-sm font-normal hover:text-neutral-400 transition-color duration-500 px-3 py-2 rounded">{items.title}</a>
                    ))}
                </div>
                <div className="flex items-center cursor-pointer">
                    ...
                    <div className="w-max h-auto overflow-hidden rounded-[0.325rem] mx-[0.6rem]">
                        <img className="profile-icon w-max h-auto object-fill" src="http://occ.a.nflxso.net/dnm/api/v6/K6hjPJd6cR6FpVELC5Pd6ovHRSk/AAAABY20DrC9-11ewwAs6nfEgb1vrORxRPP9IGmlW1WtKuaLIz8VxCx5NryzDK3_ez064IsBGdXjVUT59G5IRuFdqZlCJCneepU.png?r=229" alt=""></img>
                    </div>
